...
| connect | 数据源连接信息 | string| 是 | 无 | mysql 和 hive为 sqlachemy的连接串；kafka为broker连接串；file为文件路径|
|table| 表名 | string | 是 |  无 | 将各种数据源操作单位都抽象为表，数据库中为表，kafka中为topic，file为文件名，hbase为列族，mongo为集合|
